...
// --- 1. KATMAN: CSP (Content Security Policy) ---
// Bu ayar, dışarıdan gelen veya içeriye sızdırılan kodların çalışmasını TARAYICI SEVİYESİNDE engeller.
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            connectSrc: ["'self'", "https://www.youtube.com", "https://googleads.g.doubleclick.net"], // YouTube reklam/analytics bağlantıları için
            upgradeInsecureRequests: [],
        },
    },
    crossOriginEmbedderPolicy: false, // YouTube iframe hatası için kapalı
}));
...
// HATA YAKALAMA (Sunucunun çökmesini engeller)
process.on('uncaughtException', (err) => {
    console.error('KRİTİK HATA YAKALANDI (Sunucu Kapanmadı):', err);
});
...
const MAX_REQUESTS_PER_SEC = 4; // İyice kıstık
...
const registeredUsers = {}; // Mail-Şifre Kilidi
...
// --- 2. KATMAN: SANITIZATION (Temizlik) ---
function sanitize(text) {
    if (typeof text !== 'string') return "";
    // Tüm tehlikeli karakterleri HTML Entity'e çevir
    return text
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
        .replace(/'/g, "&#039;")
        .replace(/`/g, "&#96;") // Template literal saldırısı için
        .replace(/\//g, "&#x2F;") 
        .substring(0, 200); // Max karakter sınırı
}
...
io.on('connection', (socket) => {
    try {
        ...
        users[socket.id] = { 
            name: "Doğrulanmamış", email: "-", password: "-", 
        ...
        socket.on('set_user_data', (data) => {
            ...
            // E-Posta / Şifre Kilitleme Mantığı
            ...
            if (users[socket.id]) {
                ...
                const sysMsg = { user: 'SİSTEM', text: `🟢 ${safeName} sisteme giriş yaptı.`, type: 'system' };
        ...
        socket.on('admin_girisi', (sifre) => {
            ...
            if (typeof sifre !== 'string' || sifre !== ADMIN_SIFRESI) {
                ...
                if (loginAttempts[clientIP].count >= MAX_LOGIN_ATTEMPTS) {
                    ...
                    socket.emit('admin_error', `⛔ 5 dakika kilitlendiniz.`);
                } else {
            ...
            socket.emit('chat_message', { user: 'SİSTEM', text: `🛡️ Yönetici yetkisi alındı.`, type: 'system' });
        ...
        socket.on('video_action', (data) => {
            if (admins.has(socket.id)) {
                ...
                if(data.videoId) data.videoId = sanitize(data.videoId); // ID temizliği
        ...
        console.error("Soket Hatası:", e);
    }
});

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => { console.log(`MILITARY GRADE SECURITY ACTIVE: Port ${PORT}`); });