...
// --- YENİ: KAYITLI KULLANICI HAFIZASI ---
// { 'ahmet@gmail.com': 'sifre123' } şeklinde tutar.
...
io.on('connection', (socket) => {
    ...
    socket.on('set_user_data', (data) => {
        ...
        if (!data.name || !data.email || !data.password || !data.name.trim()) {
            socket.emit('login_failed', 'Eksik bilgi girdiniz.');
            return;
        }

        // Temizleme
        let safeName = sanitize(data.name).substring(0, 15);
        let safeEmail = sanitize(data.email).substring(0, 80).toLowerCase(); // E-postayı küçük harf yap
        let safePass = sanitize(data.password).substring(0, 30);

        // --- KRİTİK KONTROL: BU MAİL DAHA ÖNCE KAYDEDİLDİ Mİ? ---
        if (registeredUsers[safeEmail]) {
            // Evet, kayıtlı. Peki şifre doğru mu?
            if (registeredUsers[safeEmail] !== safePass) {
                // Hayır, şifre yanlış!
                console.log(`⚠️ Hatalı Şifre Denemesi: ${safeEmail} (IP: ${clientIP})`);
                socket.emit('login_failed', '⛔ Bu e-posta adresi için yanlış şifre girdiniz!');
                return; // Girişi durdur
            }
            // Şifre doğru, devam et...
        } else {
            // Hayır, bu mail ilk defa geliyor. Şifreyi kilitle!
...
server.listen(PORT, () => { console.log(`SECURE AUTH MODE: Port ${PORT}`); });